								  ...
								  <table border=1 cellpadding="5" cellspacing="1" >
									...
									<table>
										<tr>
											...
											<td align="center"><b><c:if test="${sessionScope.uid!=9999}">결재/보류일</c:if><c:if test="${sessionScope.uid==9999}">최근결재시간</c:if></td>
										...
										<c:forEach var="xxx" items="${requestScope.salesList}" varStatus="loopTagStatus">
												...
												<c:if test="${sessionScope.uid==9999 && xxx.last_sign_code==-1}"><span style="color:#66CDAA">결재 진행중</span></c:if>	
												<c:if test="${sessionScope.uid==9999 && xxx.last_sign_code==0}"><span style="color:#6B8E23">결재 완료</span></c:if>	
												<c:if test="${sessionScope.uid==9999 && xxx.last_sign_code==1}"><span style="color:#DC143C">반려</span></c:if>	
											<td align="center"><c:if test="${sessionScope.uid!=9999}">${xxx.next_app_getSign_date}
												...
												<c:if test="${sessionScope.uid==9999}">${xxx.last_sign_date}</c:if>
												<c:if test="${sessionScope.uid==9999 &&xxx.last_sign_date==null}">${xxx.next_app_getSign_date}</c:if>
